...
本文件的完整交付內容、TDD 證據、驗證基線、Drive 狀態、範圍審核與已知限制，已整理於本功能 PR #8。最終同樹 CI Run 編號與 merge SHA 以 PR 紀錄為準。

## 核心交付

- HTTPS 網路收音機與常駐 MediaProvider。
- YouTube 官方可見 iframe，支援影片與播放清單。
- 收音機與 YouTube 互斥播放。
- 打地鼠期間將有效音量降至最新基準音量的 30%。
- 玩家自訂內容、收藏、排序與刪除。
...
最新文件提交後仍須以 PR 最新 HEAD 再執行同樹 CI，成功且 `behind_by = 0` 才可合併。

## Drive 與發布限制

Drive 尚無核准的正式電台或 YouTube 來源，因此：

```text
data/media/default-library.json = []
```

本次沒有加入測試網址或來源不明內容。正式發布前仍需補齊核准來源、Android／iOS 真機播放、YouTube 嵌入允許與不同瀏覽器安全區版面證據。
